...
impl AnisetteData {
    pub async fn new() -> Result<Self, Error> {
        // native_anisette talks to akd over XPC (a blocking call), so keep it off the reactor.
        let base_headers = tokio::task::spawn_blocking(native_anisette::base_headers)
            .await
            .map_err(|_| Error::DeveloperSessionRequestFailed)??;
